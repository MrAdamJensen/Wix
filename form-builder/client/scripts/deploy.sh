...
uglify -s login_app_bundle.js -o __deployme/login_app_bundle.js
...
cp login.html __deployme/login.html
...
cat __deployme/login.html | sed 's/<!-- load here static -->/{% load static %}/g' | sed "s/bundle.css/{% static  'css\/bundle.css' %}/g" | sed "s/login_app_bundle.js/{% static  'js\/login_app_bundle.js' %}/g" > ../server/form_builder_server/form_builder/templates/registration/login.html
